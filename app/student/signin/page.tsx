...
function StudentLoginForm() {
  ...
  useEffect(() => {
    ...
    // Invalid/expired cookies are cleared so the student can sign in again.
    const checkExistingSession = async () => {
      try {
        ...
        // network error — let the student attempt login normally
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // This page is for students only — reject staff accounts
      const meRes = await fetch("/api/auth/me");
      if (meRes.ok) {
        const payload = await meRes.json();
        if (payload?.data?.role !== "STUDENT") {
          await fetch("/api/auth/session", { method: "DELETE" });
          // Also drop the client-side Firebase session so no stale auth state lingers
          await signOut(auth);
          throw new Error("This sign-in is for students only. Staff should use the main sign-in page.");
        }
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-navy-950/5 px-4 py-12">
      <div className="w-full max-w-md rounded-xl border border-navy-950/10 bg-white p-8 shadow-card">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-6 font-display text-3xl font-semibold text-navy-950">Student Sign In</h1>
          <p className="mt-2 text-sm text-navy-950/60">RNS First Grade College — Student Portal</p>
        </div>
